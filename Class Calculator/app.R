...
overflow-y:scroll; max-height: 50px; background: ghostwhite;
                  ...
                  mainPanel(navbarPage('Class Calculator', id = "inTabset", 
                                       ...
                                       tabPanel(title= "Your Result", value= "panel9",
                                                ...
                                                conditionalPanel(
                                                  condition = "input.GBCSR == '1'",
                                                  p("These results cannot give you an exact account of your social class but can select from the maximum probability of class location. For Clarification, Class1= New Affluent Worker, Class2= Emergent Service Worker, Class3= Elite, Class4= Precariat, Class5= Traditional Middle Class, Class6= Established Service Worker, and Class7= Traditional Working Class. Your Social Class as determined by GBCSR is:"),
                                                  textOutput("probabilityresults"),
                                                ),
                                                conditionalPanel(
                                                  condition = "input.WrightF == '1'",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit, {
    ...
    # Output the class number and its corresponding probability
    output$probabilityresults <- renderText({
      paste("Class with the highest probability for observation 1027:")
      paste("Class:", max_prob_index, ", Probability:", max_prob)
    })
  })
